import os

def generate_social_svg(filename, label, icon_path, color="#00f2ff"):
    svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="180" height="48" viewBox="0 0 180 48">
    <defs>
        <linearGradient id="grad{label}" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" style="stop-color:rgba(0, 242, 255, 0.05);stop-opacity:1" />
            <stop offset="100%" style="stop-color:rgba(0, 242, 255, 0);stop-opacity:1" />
        </linearGradient>
    </defs>
    
    <!-- Card Background -->
    <rect width="178" height="46" x="1" y="1" rx="6" fill="#0d1117" stroke="#30363d" stroke-width="1" />
    <rect width="178" height="46" x="1" y="1" rx="6" fill="url(#grad{label})" />
    
    <!-- Icon Container -->
    <circle cx="28" cy="24" r="14" fill="rgba(0, 242, 255, 0.1)" />
    
    <!-- Icon -->
    <path d="{icon_path}" fill="{color}" transform="translate(18, 14) scale(0.8)" />
    
    <!-- Text -->
    <text x="54" y="29" font-family="'Segoe UI', 'Inter', sans-serif" font-size="14" font-weight="600" fill="#c9d1d9">{label}</text>
    
    <!-- Hover Glow Effect (Static simulation) -->
    <rect x="1" y="1" width="178" height="46" rx="6" fill="none" stroke="{color}" stroke-opacity="0.2" stroke-width="1" />
</svg>"""
    
    with open(filename, "w", encoding="utf-8") as f:
        f.write(svg)
    print(f"Generated {filename}")

# Icon Paths (Simplified)
icons = {
    "GitHub": "M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z",
    "LinkedIn": "M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z",
    "Portfolio": "M12 0c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm1 16.057v-3.057h-2v3.057c-2.086-.339-3.822-1.776-4.576-3.708l1.838-.724c.488 1.254 1.615 2.189 2.738 2.404v-4.029h2v4.029c1.123-.215 2.25-1.15 2.738-2.404l1.838.724c-.753 1.932-2.49 3.369-4.576 3.708z",
    "Email": "M12 12.713l-11.985-9.713h23.97l-11.985 9.713zm0 2.574l-12-9.725v15.438h24v-15.438l-12 9.725z"
}

# Generate files
generate_social_svg("h:/profile/btn_github.svg", "GitHub", icons["GitHub"])
generate_social_svg("h:/profile/btn_linkedin.svg", "LinkedIn", icons["LinkedIn"])
generate_social_svg("h:/profile/btn_portfolio.svg", "Portfolio", icons["Portfolio"])
generate_social_svg("h:/profile/btn_email.svg", "Email", icons["Email"])
